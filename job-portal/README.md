...
**Backend** — package a runnable jar:
```bash
cd backend
mvn clean package
...
host (Nginx, Vercel, Netlify, etc.). Update the `baseURL` in `src/api/axios.js` to
point at your deployed backend URL before building.

---

## 8. Ideas to extend this for your resume

- Add resume upload (store file + metadata, e.g. via S3 or local disk)
- Add pagination and sorting to the job search endpoint
- Add email notifications when an application's status changes
- Add a "saved jobs" feature for candidates
- Write unit tests (JUnit + Mockito) for the service layer
- Containerize both services with Docker Compose
- Add Swagger/OpenAPI docs (`springdoc-openapi-starter-webmvc-ui`)
- Deploy backend to Render/Railway and frontend to Vercel/Netlify, and put the live
  link + architecture notes in your README
